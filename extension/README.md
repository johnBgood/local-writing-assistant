...
Download the [Mac app DMG](https://github.com/johnBgood/local-writing-assistant/releases/download/v0.4.3/LocalWriter-0.4.3-arm64-beta.dmg) and [extension ZIP](https://github.com/johnBgood/local-writing-assistant/releases/download/v0.4.3/LocalWriter-Chrome-0.4.3.zip) from GitHub. Install the app, set up Ollama/Qwen3 once, then choose **Install Chrome Bridge…** in the app menu. Extract the ZIP to a permanent folder and load it through **chrome://extensions → Developer mode → Load unpacked**. No source checkout or build tools are required. Follow the [complete setup guide](../docs/TESTERS.md).
